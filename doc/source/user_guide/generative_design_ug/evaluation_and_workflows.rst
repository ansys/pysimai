...
- **Parameter Importance**: how much each latent dimension contributes to the variability of the
  training data. Useful when working with a reduced latent space (see :ref:`reduced_latent_space_geomai`).
...
.. tip::
    If a geometry consistently appears as an outlier across retraining runs, consider removing it
    from the dataset and assessing whether its shape is representative of the design space you want
    to explore.
...
Each geometry in your training set has corresponding latent parameters, and new
geometries are generated by specifying new positions in the latent space.
...
- You can retrieve the latent parameters of all training geometries with
  ...
  to truncate the latent parameters to its first ``n`` (most important) dimensions;
  see :ref:`reduced_latent_space_geomai`.
...
Choosing the right number of latent dimensions
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The MER includes a **Parameter Importance** section showing how much each latent dimension
contributes to the variability of the training data. Use it to decide how many latent parameters
to keep.

For example, if the first 10 latent parameters account for most of the variance,
working with ``n=10`` gives a compact space with minimal loss of geometric fidelity.

Retrieving reduced latent parameters
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
.. code-block:: python
    ...
    # Only the 10 most important latent parameters
    reduced = workspace.get_latent_parameters(n=10)

When ``n`` is omitted, the full list of latent parameters is returned.

Generating geometries from a reduced space
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Reduced latent parameters can be passed directly to :class:`GeomAIPredictionConfiguration
...
.. note::
    Reducing the number of latent parameters discards the information carried by the removed
    dimensions. Geometries reconstructed from a reduced space are close approximations of the originals,
...
- Stay close to known latent parameters. Use the
...
- If you are specifying latent parameters manually, check that all values stay within the
  approximate range of the ones from your training data (typically -3 to +3). Values far
  outside this range land outside of the region learned by the model.
...
- Increase the resolution parameter. The default automatic resolution may be insufficient for
...
- The model can only interpolate meaningfully within the region it has learned. If the path between
  ...
  the design space.
- Add intermediate geometries as training data between the two endpoints and retrain your model.
  This gives the model concrete anchor points to learn a meaningful interpolation path.

Model results do not improve after retraining
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
- Remove persistent outliers: if the MER consistently flags the same geometries across builds,
  ...
  model quality.
- Make sure that you are comparing models using the same preset. Comparing a ``short`` build against a
...
convergence. See :ref:`reduced_latent_space_geomai` for how to retrieve reduced
latent parameters and use them as design variables.
